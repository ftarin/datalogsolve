...
CAESAR_TYPE_INDEX_TABLE_1 CAESAR_DATALOG_GET_INDEX_BY_VARIABLE(CAESAR_TYPE_STRING VARIABLE)
{
     ...
     SUCCESS = CAESAR_SEARCH_TABLE_1(VARIABLE_TABLE, (CAESAR_TYPE_POINTER) &VARIABLE, &INDEX, &ENTRY);
...
void CAESAR_DATALOG_BUILD_VARIABLE_TABLE()
{
     ...
     CAESAR_BEGIN_DEBUG(2) {
       DEBUG_CREATE_TABLE("VARIABLE", BASE_AREA, MARK_AREA);
     } CAESAR_END_DEBUG;
	...
	table as needed... */
     ...
     for (RULE_CELL = CAESAR_DATALOG_RULE_LIST;
	  RULE_CELL != NULL;
	  RULE_CELL = RULE_CELL->NEXT) {
	  
	  RULE = RULE_CELL->RULE;

	  ATOM = RULE->HEAD;
	  
	  for (VARIABLE_CELL = ATOM->RULE_VARIABLE_LIST;
	       VARIABLE_CELL != NULL;
	       VARIABLE_CELL = VARIABLE_CELL->NEXT) {
	       ...
	       *((CAESAR_TYPE_STRING *)BASE) = VARIABLE;
	       ...
	       CAESAR_SEARCH_AND_PUT_TABLE_1(AUX_VARIABLE_TABLE, &INDEX, &EXISTING_ENTRY_POINTER);
	  }
	  
	  for (LITERAL_CELL = RULE->TAIL;
	       LITERAL_CELL != NULL;
	       LITERAL_CELL = LITERAL_CELL->NEXT
	       ) {
	       
	       LITERAL = LITERAL_CELL->LITERAL;
	       
	       for (VARIABLE_CELL = LITERAL->RULE_VARIABLE_LIST;
		    VARIABLE_CELL != NULL;
		    VARIABLE_CELL = VARIABLE_CELL->NEXT) {
