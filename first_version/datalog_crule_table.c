...
void CAESAR_DATALOG_BUILD_CRULE_I_TABLE(CAESAR_TYPE_TABLE_1 *CRULE_I_TABLE_POINTER, CAESAR_TYPE_NATURAL RULE_LENGTH)
{
     ...
     CAESAR_BEGIN_DEBUG(2) {
       DEBUG_CREATE_TABLE("CRULE_I", BASE_AREA, MARK_AREA);
     } CAESAR_END_DEBUG;
...
void CAESAR_DATALOG_BUILD_CRULE_TABLE()
{
     ...
     CAESAR_BEGIN_DEBUG(2) {
       DEBUG_CREATE_TABLE("CRULE", BASE_AREA, MARK_AREA);
     } CAESAR_END_DEBUG;
...
/*      CAESAR_TYPE_NATURAL RULE_BODY_LENGTH; */
/*      CAESAR_TYPE_TABLE_1 CRULE_TABLE_I; */
     
/*      for (CELL = CAESAR_DATALOG_RULE_LIST; CELL->NEXT != NULL; CELL = CELL->NEXT) { */
	  
/* 	  RULE_BODY_LENGTH = CAESAR_LITERAL_LIST_LENGTH(CELL->RULE->TAIL); */
...
void CAESAR_DATALOG_PRINT_CRULE_BY_INDEX(CAESAR_TYPE_INDEX_TABLE_1 INDEX)
{
     ...
     i = 0;
     if (i < RULE->LENGTH) {
	  CAESAR_DATALOG_GET_CPREDICATE_BY_INDEX(CRULE->CPREDICATE_INDICES[i], CPREDICATE);
