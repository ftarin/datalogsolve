...
CAESAR_TYPE_NATURAL CAESAR_DATALOG_FACT_TABLE_ENTRY_SIZE(CAESAR_TYPE_NATURAL ARITY)
{
...
void CAESAR_DATALOG_EDIT_FACT_TABLE_ENTRY(
     ...
     )
{
     ...
     ((CAESAR_TYPE_INDEX_TABLE_1 *) FACT_TABLE_ENTRY)[INDEX] = TABLE_INDEX;
}

/*------------------------------------------------------------------------------*/
/* Get an entry's field of the fact table */

void CAESAR_DATALOG_GET_FACT_TABLE_ENTRY(
     CAESAR_TYPE_POINTER FACT_TABLE_ENTRY,
     CAESAR_TYPE_NATURAL INDEX,
     CAESAR_TYPE_NATURAL ARITY,
     CAESAR_TYPE_INDEX_TABLE_1 *TABLE_INDEX
...
void CAESAR_DATALOG_PRINT_FACT_TABLE(
     ...
     )
{
     ...
     for (TABLE_INDEX = 0; TABLE_INDEX < MAXIMUM; TABLE_INDEX++) {
	  ...
	  for (ENTRY_INDEX = 0; ENTRY_INDEX < ARITY; ENTRY_INDEX++) {
	       ...
	       CAESAR_DATALOG_GET_FACT_TABLE_ENTRY(
		    ENTRY,
		    ENTRY_INDEX,
		    ARITY,
		    &ENTRY_MEMBER_INDEX
     ...
     )
{
     ...
     return CAESAR_SEARCH_AND_PUT_TABLE_1(FACT_TABLE, INDEX, &EXISTING_ENTRY_POINTER);
}

/*------------------------------------------------------------------------------*/
/* Create an entry to add to a fact table in the future */

void CAESAR_DATALOG_CREATE_FACT_TABLE_ENTRY(
     CAESAR_TYPE_TABLE_1 FACT_TABLE,
     CAESAR_TYPE_POINTER *FACT_TABLE_ENTRY)
{
...
void CAESAR_DATALOG_BUILD_FACT_TABLE(CAESAR_TYPE_TABLE_1 *FACT_TABLE_POINTER, CAESAR_TYPE_NATURAL ARITY, CAESAR_TYPE_STRING RELATION_NAME)
{
     ...
     CAESAR_BEGIN_DEBUG(2) {
       DEBUG_CREATE_TABLE("FACT", BASE_AREA, MARK_AREA);
     } CAESAR_END_DEBUG;
...

     {
	  ...
	  if ((FACT_FILE = fopen(FACT_FILENAME, "r")) == NULL) {
	       CAESAR_ERROR("File ``%s'' (input tuples) is unreadable ", FACT_FILENAME);
	  }
	  ...
	  {  /* This bloc is for documenting purposes */
	       ...
	       while (getline(&STRING_BUFFER, &STRING_BUFFER_SIZE, FACT_FILE) > 0) {
		    ...
		    CAESAR_DATALOG_EDIT_FACT_TABLE_ENTRY(FACT_ENTRY, 0 ,ARITY, INDEX);
		    
		    for (i = 1; i < ARITY; i++) {
			 FIELD = strtok(NULL, FIELD_DELIMITERS);
			 INDEX = (CAESAR_TYPE_INDEX_TABLE_1) atol(FIELD);
			 CAESAR_DATALOG_EDIT_FACT_TABLE_ENTRY(FACT_ENTRY, i ,ARITY, INDEX);
		    }
...
CAESAR_TYPE_BOOLEAN CAESAR_DATALOG_IS_FACT(CAESAR_TYPE_TABLE_1 FACT_TABLE, CAESAR_TYPE_POINTER FACT_TABLE_ENTRY)
{
     CAESAR_TYPE_INDEX_TABLE_1 INDEX;
     CAESAR_TYPE_POINTER ENTRY;
