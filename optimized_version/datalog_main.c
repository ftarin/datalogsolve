...
int main(int argc, char ** argv)
{
	...
	CAESAR_READ_DATALOG_1(&DATALOG_BES, DATALOG_FILENAME);
	if (DATALOG_BES == NULL)
	...
	if (caesar_output_bes == CAESAR_TRUE)
	{
		...
		{
			CAESAR_CREATE(caesar_output_bes_file_name,
					strlen(DATALOG_SOLVE_BES_SUFIX) + strlen(DATALOG_FILENAME)
							+ 1, CAESAR_TYPE_STRING);
			strcpy(caesar_output_bes_file_name, DATALOG_FILENAME);
			strcat(caesar_output_bes_file_name, DATALOG_SOLVE_BES_SUFIX);
		}

		if ((DATALOG_BES_FILE = fopen(caesar_output_bes_file_name, "w"))
				== NULL)
		{
			CAESAR_ERROR("File ``%s'' is unwritable",
					caesar_output_bes_file_name);
		}

		CAESAR_WRITE_BES_DATALOG_1(DATALOG_BES, DATALOG_BES_FILE);
		fprintf(stdout,"%s\n", caesar_output_bes_file_name);
	}
