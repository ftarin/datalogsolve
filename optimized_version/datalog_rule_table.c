...
void CAESAR_DATALOG_BUILD_RULE_TABLE()
{
	...
	CAESAR_CREATE_TABLE_1( &RULE_TABLE_AUX, BASE_AREA, MARK_AREA, 0, /* LIMIT_SIZE by default */
	...
	for (RLCELL = CAESAR_DATALOG_RULE_LIST; RLCELL != NULL; RLCELL
			= RLCELL->NEXT)
	{

		INDEX
				= CAESAR_DATALOG_GET_RELATION_INDEX_BY_NAME(RLCELL->RULE->HEAD->RELATION->NAME);
		...
		CAESAR_DATALOG_BUILD_CRULE_I_TABLE(&(RULE_ENTRY.CRULE_TABLE),
				RULE_ENTRY.LENGTH);
		CAESAR_DATALOG_ADD_RULE_TABLE_ENTRY(&RULE_ENTRY, &(RULE_I.RULE_INDEX));
		CAESAR_DATALOG_ADD_RULE_I_TABLE_ENTRY(R_ENTRY->RULES, &RULE_I,
				&RULE_I_INDEX);
...
CAESAR_TYPE_BOOLEAN CAESAR_DATALOG_ADD_RULE_TABLE_ENTRY(
		CAESAR_TYPE_RULE_TABLE_ENTRY RULE, CAESAR_TYPE_INDEX_TABLE_1 *INDEX)
{
	...
	RULE_BASE
			= (CAESAR_TYPE_RULE_TABLE_BASE) CAESAR_PUT_BASE_TABLE_1(RULE_TABLE);
	RULE_BASE->RULE = RULE->RULE;

	RULE_MARK
			= (CAESAR_TYPE_RULE_TABLE_MARK) CAESAR_PUT_MARK_TABLE_1(RULE_TABLE);
	...
	for (ARGUMENT_INDEX = 0, VCELL2 = RULE->RULE->HEAD->RULE_VARIABLE_LIST; VCELL2
			!= NULL; ARGUMENT_INDEX++, VCELL2 = VCELL2->NEXT)
	{

		LINKED_VARIABLES = CAESAR_CONSTRUCT_RULE_VARIABLE_LIST(
				VCELL2->RULE_VARIABLE, LINKED_VARIABLES);
	...
	for (LITERAL_INDEX = 0, LCELL = RULE->RULE->TAIL; LCELL != NULL;
			LITERAL_INDEX++, LCELL = LCELL->NEXT)
	{
		...
		TAIL_TO_CONCRETIZE_RELATION_INDICES[LITERAL_INDEX]
				= CAESAR_DATALOG_GET_RELATION_INDEX_BY_NAME(LCELL->LITERAL->RELATION->NAME);
		...
		TAIL_TO_CONCRETIZE_SIZE
				+= COMBINATION_SIZE(LCELL->LITERAL->RELATION->ARITY);

		/*======================================================*/
		/* Iterate through the arguments of the current literal */
		/*======================================================*/

		for (ARGUMENT_INDEX = 0, VCELL2 = LCELL->LITERAL->RULE_VARIABLE_LIST; VCELL2
				!= NULL; ARGUMENT_INDEX++, VCELL2 = VCELL2->NEXT)
		{
			...
			if (CAESAR_RULE_VARIABLE_IN_LIST(VCELL2->RULE_VARIABLE,
					LINKED_VARIABLES) == CAESAR_FALSE
					&& CAESAR_RULE_VARIABLE_IN_LIST(VCELL2->RULE_VARIABLE,
							FREE_VARIABLES) == CAESAR_FALSE)
			{

				FREE_VARIABLES = CAESAR_CONSTRUCT_RULE_VARIABLE_LIST(
						VCELL2->RULE_VARIABLE, FREE_VARIABLES);
	...
	RULE_MARK->TAIL_TO_CONCRETIZE_RELATION_INDICES
			= TAIL_TO_CONCRETIZE_RELATION_INDICES;
	...
	for (VCELL = LINKED_VARIABLES; VCELL != NULL; VCELL = VCELL->NEXT)
	{

		TAIL_POSITION_LIST = NULL;

		for (VCELL2 = RULE->RULE->HEAD->RULE_VARIABLE_LIST, ARGUMENT_INDEX = 0; VCELL2
				!= NULL; VCELL2 = VCELL2->NEXT, ARGUMENT_INDEX++)
		{
			if (0 == strcmp(VCELL2->RULE_VARIABLE, VCELL->RULE_VARIABLE))
			{
				HEAD_POSITION = CAESAR_CREATE_VARIABLE_POSITION( 0, /* There is only one atom in the head */
				ARGUMENT_INDEX);
			}
		}
		for (LCELL = RULE->RULE->TAIL, LITERAL_INDEX = 0; LCELL != NULL; LCELL
				= LCELL->NEXT, LITERAL_INDEX++)
		{
			for (VCELL2 = LCELL->LITERAL->RULE_VARIABLE_LIST, ARGUMENT_INDEX
					= 0; VCELL2 != NULL; VCELL2 = VCELL2->NEXT,
					ARGUMENT_INDEX++)
			{
				if (0 == strcmp(VCELL2->RULE_VARIABLE, VCELL->RULE_VARIABLE))
				{
					TAIL_POSITION = CAESAR_CREATE_VARIABLE_POSITION(
							LITERAL_INDEX, ARGUMENT_INDEX);
					TAIL_POSITION_LIST
							= CAESAR_CONSTRUCT_VARIABLE_POSITION_LIST(
									TAIL_POSITION, TAIL_POSITION_LIST);
				}
			}
		}
		...
		TAIL_POSITION_LIST
				= CAESAR_REVERSE_VARIABLE_POSITION_LIST(TAIL_POSITION_LIST);

		VINDEX = VARIABLE_TABLE_GET_INDEX_BY_NAME(VCELL->RULE_VARIABLE);

		MAP = CAESAR_CREATE_VARIABLE_MAP(VINDEX, /* Variable index */
		0, /* Not necessary */
	...
	for (VCELL = FREE_VARIABLES; VCELL != NULL; VCELL = VCELL->NEXT)
	{
		...
		for (LCELL = RULE->RULE->TAIL, j = 0; LCELL != NULL && j
				<= TAIL_POSITION_LIST->POSITION->LITERAL_INDEX; LCELL
				= LCELL->NEXT, j++)
		{
			...
			if (j == TAIL_POSITION_LIST->POSITION->LITERAL_INDEX)
			{
				...
				for (DCELL = LCELL->LITERAL->RELATION->DOMAIN_LIST, k = 0; DCELL
						!= NULL && k
						<= TAIL_POSITION_LIST->POSITION->ARGUMENT_INDEX; DCELL
						= DCELL->NEXT, k++)
				{

					if (k == TAIL_POSITION_LIST->POSITION->ARGUMENT_INDEX)
					{
						DOMAIN_TABLE_GET_FILTERED_CARDINALITY_BY_NAME(
								DCELL->DOMAIN->NAME, &CARDINAL);
					}
		...
		fprintf(stdout, "%d ", CARDINAL);
		
		MAP = CAESAR_CREATE_VARIABLE_MAP(VINDEX, /* Variable index */
		CARDINAL, HEAD_POSITION, /* Head position */
		TAIL_POSITION_LIST /* Tail positions */
		);
	...
	CAESAR_VARIABLE_MAP_LIST_TO_ARRAY(RULE_MARK->FREE_VARIABLE_MAP_LIST,
			&(RULE_MARK->FREE_VARIABLE_NUMBER),
			&(RULE_MARK->FREE_VARIABLE_MAP_ARRAY));

	/* /CALCULATE THE FREE AND LINKED VARIABLES */

	return !CAESAR_SEARCH_AND_PUT_TABLE_1(RULE_TABLE, INDEX,
...
void CAESAR_DATALOG_GET_RULE_BY_INDEX(CAESAR_TYPE_INDEX_TABLE_1 INDEX,
		CAESAR_TYPE_RULE_TABLE_ENTRY ENTRY)
{
	CAESAR_TYPE_RULE *BASE;
	CAESAR_TYPE_RULE_TABLE_MARK MARK;

	CAESAR_RETRIEVE_I_B_TABLE_1(RULE_TABLE, INDEX,
			(CAESAR_TYPE_POINTER *) &BASE);

	ENTRY->RULE = *BASE; /* TODO: CHECK THIS POINTERS */

	CAESAR_RETRIEVE_I_M_TABLE_1(RULE_TABLE, INDEX,
	...
	ENTRY->TAIL_TO_CONCRETIZE_RELATION_INDICES
			= MARK->TAIL_TO_CONCRETIZE_RELATION_INDICES;
...
void CAESAR_DATALOG_BUILD_RULE_I_TABLE(
		CAESAR_TYPE_TABLE_1 *RULE_I_TABLE_POINTER)
{
	...
	CAESAR_CREATE_TABLE_1( &RULE_I_TABLE_AUX, BASE_AREA, MARK_AREA, 0, /* LIMIT_SIZE by default */
	0, /* HASH_SIZE by default */
	CAESAR_TRUE, /* Sets HASH_SIZE to its nearest prime number  */
	NULL, /* COMPARE_FUNCTION */
	NULL, /* HASH_FUNCTION *//* TODO: PUT THE CAESAR_7_HASH */
...
CAESAR_TYPE_BOOLEAN CAESAR_DATALOG_ADD_RULE_I_TABLE_ENTRY(
		CAESAR_TYPE_TABLE_1 RULE_I_TABLE,
		CAESAR_TYPE_RULE_I_TABLE_ENTRY RULE_I, CAESAR_TYPE_INDEX_TABLE_1 *INDEX)
{
	CAESAR_TYPE_RULE_I_TABLE_BASE RULE_I_BASE;
	CAESAR_TYPE_POINTER EXISTING_ENTRY_POINTER;

	RULE_I_BASE
			= (CAESAR_TYPE_RULE_I_TABLE_BASE) CAESAR_PUT_BASE_TABLE_1(RULE_I_TABLE);
	RULE_I_BASE->RULE_INDEX = RULE_I->RULE_INDEX;

	return !CAESAR_SEARCH_AND_PUT_TABLE_1(RULE_I_TABLE, INDEX,
			&EXISTING_ENTRY_POINTER);
}
...
void CAESAR_DATALOG_GET_RULE_I_BY_INDEX(CAESAR_TYPE_TABLE_1 RULE_I_TABLE,
		CAESAR_TYPE_INDEX_TABLE_1 INDEX, CAESAR_TYPE_RULE_I_TABLE_ENTRY ENTRY)
{
	CAESAR_TYPE_RULE_I_TABLE_BASE BASE;

	CAESAR_RETRIEVE_I_B_TABLE_1(RULE_I_TABLE, INDEX,
			(CAESAR_TYPE_POINTER *) &BASE);
